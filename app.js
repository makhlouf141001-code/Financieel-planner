...
function load() {
  ...
  try {
    ...
    if (!obj.monthCursor) obj.monthCursor = new Date().toISOString().slice(0, 7);
    if (typeof obj.income !== "number") obj.income = Number(obj.income) || 0;
    return obj;
  } catch {
...
function openModal(mode = "item") {
  // Safari: hidden alleen is soms niet genoeg → force display
...
function closeModal() {
  // Safari: force sluiten (hidden + display none)
...
// Buttons altijd hard sluiten
el("closeModal").addEventListener("click", forceClose);
el("cancelBtn").addEventListener("click", forceClose);
el("incomeCancelBtn").addEventListener("click", forceClose);

// Klik buiten de modal = sluiten (Safari-safe: check contains)
function outsideClickHandler(e) {
  const modal = modalBack.querySelector(".modal");
  if (!modal) return closeModal();
  if (!modal.contains(e.target)) closeModal();
}
modalBack.addEventListener("click", outsideClickHandler);
modalBack.addEventListener("touchstart", outsideClickHandler, { passive: true });
...
function renderDashboard() {
    ...
    for (const x of arr.slice(0, 6)) {
      ...
      row.innerHTML = `
        <div class="catname" style="opacity:.95">
          <span class="badge">${escapeHtml(x.cat)}</span>
        </div>
  ...
  segfooter.textContent =
    income === 0
      ...
      : `Uitgaven: ${fmtEUR(totalExpenses)} • Over: ${fmtEUR(income - totalExpenses)} • Maand: ${labelMonth(
          yyyyMm
        )}`;

  // Upcoming list
  const today = new Date();
  const upcoming = events.filter((e) => e.date >= startOfDay(today)).slice(0, 8);
    ...
    div.innerHTML = `
      <div>
        ...
        <div class="itemmeta">
          ...
          ${it.date ? `<span class="badge">${escapeHtml(it.date)}</span>` : ""}
          ${activeBadge}
        </div>
      </div>
  ...
  try {
    ...
    state = {
      ...
      items: obj.items.map((x) => ({
        ...
        frequency: ["monthly", "weekly", "yearly", "once"].includes(x.frequency)
          ? x.frequency
          : "monthly",
...
/* ---------- Keyboard shortcut: Enter on search opens quick-add ---------- */
document.addEventListener("keydown", (e) => {
